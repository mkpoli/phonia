...
export class WasmCoreClient implements CoreClient {
  ...
  duplicateTier(annotationId: AnnotationId, tierId: TierId): Promise<AppliedChange> {
    return this.#call({ method: 'duplicateTier', annotationId, tierId });
  }
  ...
  movePoint(annotationId: AnnotationId, pointId: PointId, to: number): Promise<AppliedChange> {
    return this.#call({ method: 'movePoint', annotationId, pointId, to });
  }
  ...
  /** Encodes `[t0, t1]` of `id` reversed in time as WAV bytes at `bits`. */
  reverseSpanWav(id: AudioId, t0: number, t1: number, bits: WavBitDepth): Promise<Uint8Array> {
  ...
  /** Encodes the whole of `id` resampled to `targetHz` as WAV bytes at `bits`. */
  resampleWav(id: AudioId, targetHz: number, bits: WavBitDepth): Promise<Uint8Array> {
