...
impl WasmEngine {
    ...
    /// The LPC-smoothed spectral envelope of `[t0, t1]` as a JSON `{ freqs, db }`
    /// of parallel arrays — the all-pole curve that traces resonance peaks over
    /// the raw spectrum.
    ///
    /// # Errors
    /// Rejects when `id` names no live buffer, or when a bound is not finite.
    #[wasm_bindgen(js_name = lpcSpectrum)]
    pub fn lpc_spectrum(&self, id: u64, t0: f64, t1: f64) -> Result<String, JsError> {
        let (freqs, db) = self.inner.lpc_spectrum(AudioId::from_u64(id), t0, t1)?;
        let value = serde_json::json!({ "freqs": freqs, "db": db });
        serde_json::to_string(&value).map_err(|err| JsError::new(&err.to_string()))
    }
    ...
    /// Rejects when `id` names no live store entry, when a bound is not finite or
    /// the span is empty, or when the span cannot be encoded.
    #[wasm_bindgen(js_name = subtractMeanWav)]
    pub fn subtract_mean_wav(
        &self,
        id: u64,
        t0: f64,
        t1: f64,
        ...
        let wav = self
            .inner
            .subtract_mean_span_wav(AudioId::from_u64(id), t0, t1, bits.into())?;
        Ok(Uint8Array::from(wav.as_slice()))
    }

    /// Encodes the whole of `id` with the span `[t0, t1]` set to silence as mono
    /// WAV bytes at `bits` — the "save a recording with a part zeroed" path.
    ///
    /// # Errors
    /// Rejects when `id` names no live buffer, when a bound is not finite, or
    /// when the span is empty.
    #[wasm_bindgen(js_name = applyZeroWav)]
    pub fn apply_zero_wav(
        &self,
        id: u64,
        t0: f64,
        t1: f64,
        ...
        let wav = self
            .inner
            .zero_span_wav(AudioId::from_u64(id), t0, t1, bits.into())?;
        Ok(Uint8Array::from(wav.as_slice()))
    }
