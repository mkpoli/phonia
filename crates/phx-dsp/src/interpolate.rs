...
///
/// The sinc factor is generated by recurrence: successive terms on one side
/// differ by π in the sine's argument, so one sine per side plus one cosine
/// per term is all the trigonometry the sum needs.
#[must_use]
pub fn sinc_interpolate(y: &[f64], x: f64, depth: usize) -> f64 {
    ...
    let left_span = x - (nr - depth) as f64 + 1.0; // φ_l + depth
    let mut a = PI * (x - nl as f64);
    let mut half_sine = 0.5 * a.sin();
    let mut aa = a / left_span;
    let daa = PI / left_span;
    for &sample in y[nr - depth..=nl].iter().rev() {
        result += sample * half_sine / a * (1.0 + aa.cos());
        a += PI;
        aa += daa;
        half_sine = -half_sine;
    }
    // Right side: samples nr, nr+1, …, nl+depth at distances φ_r, φ_r+1, ….
    let right_span = (nl + depth) as f64 - x + 1.0; // φ_r + depth
    let mut a = PI * (nr as f64 - x);
    let mut half_sine = 0.5 * a.sin();
    let mut aa = a / right_span;
    let daa = PI / right_span;
    for &sample in &y[nr..=nl + depth] {
        result += sample * half_sine / a * (1.0 + aa.cos());
        a += PI;
        aa += daa;
        half_sine = -half_sine;
